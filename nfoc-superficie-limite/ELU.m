...
%Análise do ELU: verifica se a deformação dada está no ELU
...
if kx==0 && ky==0
   ...
   e_max = max(epsilon_c);
   e_min = min(epsilon_c);
   x_emax = max(x_c(e_max==epsilon_c));
   y_emax = max(y_c(e_max==epsilon_c));
   x_emin = min(x_c(e_min==epsilon_c));
   y_emin = min(y_c(e_min==epsilon_c));
   % Alguns parâmetros
   ...
   % finalmente, avaliar o ELU (curvaturas com sinal)
   ELU = 'R';
   if teta >=0 
       if e_c>=-10 && e_c<=2
            if teta <= (10+e_c)/beta1
                ELU = 'OK';
            end
       ...
       if e_c>=-10 && e_c<=20
            if teta>=e_c-7/2 && teta<=(e_c+10)/betanc
               ELU = 'OK'; 
            end
       elseif e_c>=0 && e_c<=2
           if teta>=7/2*e_c-2
